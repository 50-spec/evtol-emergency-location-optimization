...
def build_validation_text(poi_df, calls, clusters, htr, summary):
    ...
    lines = [
        ...
        "## 核心结论",
        "",
        "本数据集由高德地图兰州市 POI 提供空间锚点，并基于论文中的河谷城市、跨河、山缘难到达逻辑模拟急救呼叫。它适合用于毕业论文的数值实验和模型验证，但仍应表述为模拟数据，而非真实 120 急救历史数据。",
        "",
        "## 关键指标",
        "",
        ...
        "## 判断",
        "",
        "与旧数据相比，本数据集消除了边界点堆积和 POI 城市错位问题；难到达比例也从 100% 降至更适合论文实验的区间。若用于论文，建议在数据说明中写明：真实急救呼叫数据不可得，因此本文使用高德 POI 作为城市空间锚点，并结合地形、跨河和拥堵因素生成模拟急救需求。",
    ]
    return "\n".join(lines)
